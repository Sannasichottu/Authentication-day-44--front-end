  ...
  return (
    <section className="home">
      ...
      <article>
        ...
        <UserSlideMenu showLogin={showLogin} setShowLogin={setShowLogin} />
      </article>
    </section>
  );
}

function UserSlideMenu({ showLogin, setShowLogin }) {
  const styles = {
    transform: showLogin ? "translateY(0px)" : "translateY(-250px)",
  };
  ...
  return (
    <section style={styles} className="loginButton">
      ...
      <article>
        ...
        <button onClick={() => setShowLogin(false)}>Hide</button>
      </article>
    </section>
  );
}
